...
export const TechMatrixTable: React.FC = () => {
  ...
  return (
    <div id="tech-matrix-interactive" className="my-8 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 overflow-hidden shadow-xs">
      {/* Controls Bar */}
      <div className="p-4 bg-slate-50 dark:bg-slate-950/80 border-b border-slate-200 dark:border-slate-800 space-y-3">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-1.5 text-xs rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 min-h-[36px]"
            />
          </div>

          <label className="inline-flex items-center space-x-2 text-xs font-medium text-slate-700 dark:text-slate-300 cursor-pointer shrink-0">
            <input
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse text-xs min-w-[720px]">
          <thead>
            <tr className="border-b border-slate-200 dark:border-slate-800 bg-slate-100/70 dark:bg-slate-900/90 text-slate-600 dark:text-slate-400 font-semibold">
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60 font-sans">
            {filteredTech.map((item) => (
              <tr
                ...
                className="hover:bg-slate-50 dark:hover:bg-slate-850 transition-colors cursor-pointer group"
                onClick={() => setInspectedTech(item)}
              >
                <td className="py-2.5 px-3 font-semibold text-slate-900 dark:text-slate-100 flex items-center space-x-1.5">
                  <span>{item.name}</span>
                ...
                <td className="py-2.5 px-3 text-slate-600 dark:text-slate-300 max-w-[220px] truncate" title={item.purpose}>
                  {item.purpose}
                </td>
                ...
                <td className="py-2.5 px-2 text-center">
                  {item.freeCloud ? (
                    <span className="inline-flex p-0.5 rounded bg-blue-50 text-blue-600 dark:bg-blue-950/40 dark:text-blue-400" title="Free Cloud Quota">
                      <Check className="h-3.5 w-3.5" />
                    </span>
                  ) : (
                    <span className="text-slate-400">—</span>
                ...
                <td className="py-2.5 px-3 text-slate-600 dark:text-slate-400 text-[11px] max-w-[200px] truncate" title={item.recommendedUse}>
                  {item.recommendedUse}
                </td>
                <td className="py-2.5 px-3 font-mono text-[10px] text-slate-500 max-w-[140px] truncate" title={item.alternative}>
                  {item.alternative}
                </td>
              </tr>
            ))}
          ...
          <div className="w-full max-w-lg rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 shadow-xl space-y-4">
            ...
            <div className="space-y-2.5 text-xs">
              ...
              <div className="pt-2 flex flex-wrap gap-2">
                ...
                <span className="px-2 py-0.5 rounded text-[11px] font-medium bg-blue-100 text-blue-800 dark:bg-blue-950/40 dark:text-blue-300">
