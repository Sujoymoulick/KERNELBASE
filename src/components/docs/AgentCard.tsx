...
export const AgentCard: React.FC<AgentCardProps> = ({ agent, onSelect }) => {
  ...
  return (
    <div
      id={cardId}
      className="rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 p-5 shadow-xs hover:border-indigo-400 dark:hover:border-indigo-500/60 transition-all flex flex-col justify-between"
    >
      <div>
        <div className="flex items-start justify-between mb-3">
          <div className="flex items-center space-x-2.5">
            <div className="p-2 rounded-md bg-indigo-50 dark:bg-indigo-950/40 text-indigo-600 dark:text-indigo-400">
        ...
        <div className="space-y-2 mb-4 pt-3 border-t border-slate-100 dark:border-slate-800/80">
          <div className="flex items-center space-x-1.5 text-xs text-slate-600 dark:text-slate-300">
            <Cpu className="h-3.5 w-3.5 text-slate-400" />
            <span className="text-[11px] text-slate-400">Recommended Model:</span>
            <span className="font-mono text-[11px] font-medium text-indigo-600 dark:text-indigo-400">
      ...
      {onSelect && (
        <button
          id={`select-${cardId}`}
          onClick={onSelect}
          className="mt-2 text-xs font-medium text-indigo-600 dark:text-indigo-400 flex items-center space-x-1 hover:underline pt-2 border-t border-slate-100 dark:border-slate-800/80"
        >
